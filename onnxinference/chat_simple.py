...
def load_model(model_path: str, provider: str):
    """Load ONNX model and tokenizer with automatic CPU fallback"""
    ...
    try:
        ...
        if provider == "cuda":
            ...
            print(f"✅ Attempting to load model with CUDA provider")
        elif provider == "dml":
            config.append_provider("dml")
            print(f"✅ Attempting to load model with DirectML provider")
        else:
            print(f"ℹ️ Loading model with CPU provider")
        
        model = og.Model(config)
        tokenizer = og.Tokenizer(model)
        
        print(f"✅ Model loaded successfully on {provider.upper()}")
        st.success(f"✅ Model loaded successfully on {provider.upper()}")
        return model, tokenizer
        
    except Exception as e:
        # If GPU fails, automatically try CPU fallback
        if provider != "cpu":
            st.warning(f"⚠️ {provider.upper()} failed: {e}")
            print(f"⚠️ {provider.upper()} failed, falling back to CPU...")
            
            try:
                config = og.Config(model_path)
                config.clear_providers()
                # CPU doesn't need explicit provider in most cases
                model = og.Model(config)
                tokenizer = og.Tokenizer(model)
                
                print(f"✅ Model loaded successfully on CPU (fallback)")
                st.info(f"ℹ️ Running on CPU (GPU fallback)")
                return model, tokenizer
                
            except Exception as cpu_error:
                st.error(f"❌ CPU fallback also failed: {cpu_error}")
...
def build_prompt(system_prompt: str, user_input: str, tokenizer: og.Tokenizer) -> str:
    """Build prompt using EXACT training format from sft_new code.py"""
    
    # Match training format exactly:
    # <|user|>\n{instruction}\n<|assistant|>\n
    
    # Note: Training didn't use system prompts, but we'll prepend if provided
    if system_prompt:
...
def main():
    ...
    with st.sidebar:
        ...
        st.info(
            "⚠️ **Note:** Your fine-tuned model was trained WITHOUT system prompts. "
            "System prompts may not work as expected with your fine-tuned model."
        )
        use_system_prompt = st.checkbox("Use System Prompt (Base Model Only)", value=False)
        ...
        if use_system_prompt:
            system_prompt = st.text_area(
                ...
                height=100,
                help="Instructions for the model (only effective with base model)"
            )
        else:
            system_prompt = ""
    ...
    # Reload model if provider or path changed
    if ("model" not in st.session_state or 
        st.session_state.get("provider") != provider or
        st.session_state.get("model_path") != model_path):
        with st.spinner(f"Loading model on {provider.upper()}..."):
            ...
            if model is not None:
                st.sidebar.success(f"✅ Model loaded on {provider.upper()}")
